...
pub extern "C" fn initialize(init: Initializers) -> *mut State {
	...
	match std::panic::catch_unwind(move || {
		...
		let renderer_3d: Box<dyn ::renderer::renderer::RendererInner> =
			Box::new(::renderer::renderer::D3d11Renderer::new(
				...
				Box::new(|texture| {
					let texture = texture
						.as_any()
						.downcast_ref::<::renderer::renderer::D3d11Texture>()
						.unwrap();
		...
		log::info!(
			target: "aetherment",
			"Plugin renderer initialized (D3D11). Compat env: AETHERMENT_D3D11_COMPAT_DISABLE_DEPTH={:?}, AETHERMENT_D3D11_COMPAT_DISABLE_CULL={:?}",
			std::env::var("AETHERMENT_D3D11_COMPAT_DISABLE_DEPTH").ok(),
			std::env::var("AETHERMENT_D3D11_COMPAT_DISABLE_CULL").ok()
		);
		...
		let core = aetherment::Core::new(
			...
			backend::BackendInitializers::PenumbraIpc(backend::penumbra_ipc::PenumbraFunctions {
				...
				root_path: Box::new(move || {
					std::path::PathBuf::from((funcs.root_path)().to_string())
				}),
				mod_list: Box::new(move || {
					(funcs.mod_list)()
						.to_string()
						.split('\0')
						.map(|v| v.to_string())
				...
				set_mod_enabled: Box::new(move |collection, id, enabled| {
					(funcs.set_mod_enabled)(FfiStr::new(collection), FfiStr::new(id), enabled)
				}),
				set_mod_priority: Box::new(move |collection, id, priority| {
					(funcs.set_mod_priority)(FfiStr::new(collection), FfiStr::new(id), priority)
				}),
				set_mod_inherit: Box::new(move |collection, id, inherit| {
					(funcs.set_mod_inherit)(FfiStr::new(collection), FfiStr::new(id), inherit)
				}),
				set_mod_settings: Box::new(move |collection, id, option, suboptions| {
					(funcs.set_mod_settings)(
						FfiStr::new(collection),
						FfiStr::new(id),
						FfiStr::new(option),
						FfiStr::new(&suboptions.join("\0")),
					)
				}),
				get_mod_settings: Box::new(move |collection, id, inherit| {
					let settings =
						(funcs.get_mod_settings)(FfiStr::new(collection), FfiStr::new(id), inherit);
...
pub extern "C" fn draw(state: *mut State, d3d11_device: usize, io: Io) -> usize {
	...
	match state.renderer.draw(d3d11_device, io, |ctx| {
		egui::CentralPanel::default()
			...
			.show(ctx, |ui| state.core.draw(ui, &state.renderer_3d));
	}) {
		Ok(v) => v,
		Err(err) => {
...
#[no_mangle]
pub extern "C" fn backend_penumbraipc_modchanged(
	state: *mut State,
	typ: aetherment::modman::backend::penumbra_ipc::ModChangeType,
	collection_id: FfiString,
	mod_id: FfiString,
) {
	let state = unsafe { &mut *state };
	aetherment::modman::backend::penumbra_ipc::subscriber_modchanged(
		state.core.mod_manager.clone(),
		typ,
