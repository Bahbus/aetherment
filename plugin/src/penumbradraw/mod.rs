...
const FALLBACK_REDUCED_FEATURES: &[&str] = &[
	"Custom preset list pagination",
	"Grouped-option nested trees collapsed into sections",
	"Explicit help buttons for all hover descriptions",
];
...
impl PenumbraDraw {
	...
	pub fn settings(&mut self, ui_scale: f32, mod_id: &str) -> bool {
		...
		if !self.mod_manager.aeth_mods.contains(mod_id) {
			return false;
		};

		let mut changed = false;
		draw_fallback_mode_banner();
		if meta.options.len() > 0 {
			...
			'default: {
				for (name, value) in settings.iter() {
					if let Some(opt) = meta.options.options_iter().find(|v| v.name == *name) {
						if aetherment::modman::settings::Value::from_meta_option(opt) != *value {
							break 'default;
						}
					}
				}
				...
				'preset: for v in presets.iter() {
					for (name, value) in settings.iter() {
						match v.settings.get(name) {
							...
							None => {
								if aetherment::modman::settings::Value::from_meta_option(
									meta.options
										.options_iter()
										.find(|v| v.name == *name)
										.unwrap(),
			...
			imgui::combo(&selected_preset, "Preset", || {
				let mut set_settings =
					|values: &HashMap<String, aetherment::modman::settings::Value>| {
						for (name, value) in settings.iter_mut() {
							*value = values.get(name).map_or_else(
								|| {
									aetherment::modman::settings::Value::from_meta_option(
										meta.options
											.options_iter()
											.find(|v| v.name == *name)
											.unwrap(),
				...
				let mut delete = None;
				let total_pages =
					(presets.len() + PRESET_PAGE_SIZE.saturating_sub(1)) / PRESET_PAGE_SIZE.max(1);
				if self.preset_page_index >= total_pages && total_pages > 0 {
					self.preset_page_index = total_pages - 1;
				}
				let page_start = self.preset_page_index * PRESET_PAGE_SIZE;
				let page_end = (page_start + PRESET_PAGE_SIZE).min(presets.len());

				if total_pages > 1 {
					imgui::label(format!(
						"Preset Page {}/{}",
						self.preset_page_index + 1,
						total_pages
					));
					imgui::same_line();
					if imgui::small_button("<") && self.preset_page_index > 0 {
						self.preset_page_index -= 1;
					}
					imgui::same_line();
					if imgui::small_button(">") && self.preset_page_index + 1 < total_pages {
						self.preset_page_index += 1;
					}
				}
				...
				{
					imgui::push_id(format!("preset-row-{i}"), || {
						if imgui::button("X") {
							delete = Some(i);
						}
						draw_help(mod_id, "Delete preset");

						imgui::same_line();
						if imgui::button("^") {
							_ = clipboard_win::set_clipboard_string(&p.sharable_string());
						}
						draw_help(mod_id, "Copy preset to clipboard");
				...
				if imgui::button("Import preset from clipboard") {
					'import: {
						let Ok(clip) = clipboard_win::get_clipboard_string() else {
							break 'import;
						};
						let Some(preset) =
							aetherment::modman::settings::Preset::from_sharable_string(&clip)
						else {
							break 'import;
						};
		...
		if !matches!(
			...
			Some(aetherment::modman::meta::OptionType::Category(_))
		) {
			...
			if let aetherment::modman::meta::OptionSettings::Grouped(group) = &option.settings {
				for sub in group.options.iter() {
					for val in sub.options.iter() {
						if let aetherment::modman::meta::ValueGroupedOptionEntryType::Option(val) =
							val
						{
		...
		for option_type in meta.options.iter() {
			let option = match option_type {
				...
				aetherment::modman::meta::OptionType::Category(s) => {
			...
			if categories.len() > 1 && cur_category != categories[self.selected_category_tab] {
				continue;
			}
		...
		if changed {
			...
			backend.apply_mod_settings(
				mod_id,
				&collection_id,
				aetherment::modman::backend::SettingsType::Some(settings.clone()),
			);
...
fn draw_fallback_mode_banner() {
	imgui::label("Fallback Mode (Reduced)");
	for feature in FALLBACK_REDUCED_FEATURES {
		imgui::label(format!("- {feature}"));
	}
}
...
fn draw_option(
	ui_scale: f32,
	mod_id: &str,
	meta: &aetherment::modman::meta::Meta,
	settings: &mut aetherment::modman::settings::CollectionSettings,
	option: &aetherment::modman::meta::Option,
	name: &str,
	desc: &str,
) -> bool {
	...
	match val {
		Grouped(val) => {
			let OptionSettings::Grouped(o) = &option.settings else {
				imgui::label(format!("Invalid setting type for {setting_id}"));
				return false;
			};
			...
			if let Some(selected) = selected {
				let section_name = format!("{}: details", selected.name);
				if !imgui::collapsing_header(&section_name) {
					draw_help(mod_id, "Expand to show grouped nested options");
					return changed;
				}
				...
				for sub in selected.options.iter() {
					match sub {
						aetherment::modman::meta::ValueGroupedOptionEntryType::Category(v) => {
							if imgui::collapsing_header(v) {
								imgui::label("Section expanded");
							}
						}
