...
pub fn label(label: impl AsRef<str>) {
	...
	unsafe {
		sys::igTextUnformatted(
			label.as_ptr() as _,
			(label.as_ptr() as usize + label.len()) as _,
		)
	};
}
...
pub fn small_button(label: impl AsRef<str>) -> bool {
	let label = format!("{}\0", label.as_ref());
	unsafe { sys::igSmallButton(label.as_ptr() as _) }
}
...
pub fn selectable_value<C: PartialEq<S>, S: Into<C>>(
	current: &mut C,
	selected: S,
	...
	if N::INTEGRAL {
		...
		r = unsafe {
			sys::igSliderInt(
				label.as_ptr() as _,
				&mut v,
				range.start().to_f64() as i32,
				range.end().to_f64() as i32,
				0 as _,
				0 as _,
			)
		};
		*value = N::from_f64(v as f64);
		...
		r = unsafe {
			sys::igSliderFloat(
				label.as_ptr() as _,
				&mut v,
				range.start().to_f64() as f32,
				range.end().to_f64() as f32,
				0 as _,
				0 as _,
			)
		};
...
pub fn color_edit(
	color: &mut impl aetherment::ui_ext::ColorEditValue,
	label: impl AsRef<str>,
) -> bool {
	const FLAGS: sys::ImGuiColorEditFlags = sys::ImGuiColorEditFlags_DisplayRGB
		| sys::ImGuiColorEditFlags_DisplayHex
		| sys::ImGuiColorEditFlags_NoInputs
		| sys::ImGuiColorEditFlags_DisplayHSV
		| sys::ImGuiColorEditFlags_Uint8
		| sys::ImGuiColorEditFlags_PickerHueWheel;

	const FLAGS_ALPHA: sys::ImGuiColorEditFlags =
		FLAGS | sys::ImGuiColorEditFlags_AlphaPreviewHalf | sys::ImGuiColorEditFlags_AlphaBar;
...
pub fn collapsing_header(label: impl AsRef<str>) -> bool {
	let label = format!("{}\0", label.as_ref());
	unsafe {
