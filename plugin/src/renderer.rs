...
fn categorized_error(bucket: &str, message: impl AsRef<str>) -> Error {
	format!("{bucket}:{}", message.as_ref()).into()
}
		...
		device.CreateSamplerState(
			&D3D11_SAMPLER_DESC {
				Filter: match (
					...
					options.mipmap_mode.unwrap_or(F::Nearest),
				) {
	...
	pub unsafe fn paint_region(
		...
		for y in 0..height {
			core::ptr::copy_nonoverlapping(
				self.data[y * self.w as usize..].as_ptr(),
				pixels_ptr.add(y * (data.RowPitch / 4) as usize),
				self.w as usize,
			);
		}
...
impl Renderer {
	pub fn new(device_ptr: usize) -> Result<Self, Error> {
		unsafe {
			let device = &(device_ptr as _);
			let device = ID3D11Device::from_raw_borrowed(device)
				.ok_or_else(|| categorized_error("device", "failed borrowing device"))?;

			let mut context = None;
			device
				.CreateDeferredContext(0, Some(&mut context))
				.map_err(|e| categorized_error("context", e.to_string()))?;

			const SHADER: &str = include_str!("egui.wgsl");
			let shader =
				translate_shader(SHADER).map_err(|e| categorized_error("shader", e.to_string()))?;
			// crate::log!("{shader}");

			let mut vs = None;
			let vs_bytecode = compile_shader(&shader, "vs_main", "vs_5_0")
				.map_err(|e| categorized_error("shader", e.to_string()))?;
			device.CreateVertexShader(&vs_bytecode, None, Some(&mut vs))?;

			let mut ps = None;
			let ps_bytecode = compile_shader(&shader, "fs_main", "ps_5_0")
				.map_err(|e| categorized_error("shader", e.to_string()))?;
			device
		...
		unsafe {
			...
			if (self.last_io.width != w || self.last_io.height != h) && w > 16.0 && h > 16.0 {
				...
				self.rt = RenderTarget::new(device, w as u32, h as u32)
					.map_err(|e| categorized_error("texture", e.to_string()))?;
			}

			let primitives = self
				.handle_egui(device, io.clone(), draw)
				.map_err(|e| categorized_error("input", e.to_string()))?;

			self.d3d11_ctx
				.ClearRenderTargetView(&self.rt.view, &[0.0, 0.0, 0.0, 0.0]);
			...
			self.d3d11_ctx
				.OMSetRenderTargets(Some(&[Some(self.rt.view.clone())]), None);
			...
			self.d3d11_ctx
				.OMSetBlendState(&self.blend, Some(&[0.0; 4]), 0xFFFFFFFF);
			self.d3d11_ctx
			...
			for prim in primitives {
				self.d3d11_ctx
					.RSSetScissorRects(Some(&[windows::Win32::Foundation::RECT {
				...
				match prim.primitive {
					...
					egui::epaint::Primitive::Mesh(mesh) => {
						match mesh.texture_id {
							egui::TextureId::Managed(id) => {
								if let Some(tex) = self.textures.get(&id) {
									self.d3d11_ctx
										.PSSetSamplers(1, Some(&[Some(tex.sampler.clone())]));
									self.d3d11_ctx
										.PSSetShaderResources(0, Some(&[Some(tex.sview.clone())]));
								}
							}
							...
							egui::TextureId::User(ptr) => {
								let ptr = ptr as _;
								let view =
									ID3D11ShaderResourceView::from_raw_borrowed(&ptr).unwrap();
								self.d3d11_ctx
						...
						let vertices = mesh
							...
							.map(|v| {
								[
									v.pos.x * io.ui_scale / io.width * 2.0 - 1.0,
									1.0 - v.pos.y * io.ui_scale / io.height * 2.0,
									v.uv.x,
									v.uv.y,
						...
						self.d3d11_ctx.IASetVertexBuffers(
							0,
							1,
						...
						self.d3d11_ctx.IASetIndexBuffer(
							&self.index_buffer,
	...
	fn handle_egui(
		...
		io: crate::Io,
		draw: impl FnMut(&egui::Context),
	) -> Result<Vec<egui::ClippedPrimitive>, Error> {
		...
		if self.last_io.mouse_x != io.mouse_x || self.last_io.mouse_y != io.mouse_y {
			events.push(egui::Event::PointerMoved(egui::pos2(
				io.mouse_x / io.ui_scale,
				io.mouse_y / io.ui_scale,
			)));
		}
		...
		if io.input_buf_ptr != 0 {
			let inputs = unsafe {
				std::slice::from_raw_parts(io.input_buf_ptr as *const u16, io.input_buf_len)
			}
			.to_vec();
		...
		for (keycode, (last, now)) in self.last_io_keys.iter().zip(io_keys.iter()).enumerate() {
			if *last != *now {
				let Some(key) = convert_key(keycode as u8) else {
					continue;
				};
		...
		for (id, delta) in out.textures_delta.set {
			...
			if delta.is_whole() {
				...
				self.textures.insert(id, unsafe {
					Texture::new(device, w, h, delta.options).unwrap()
				});
			}
			...
			unsafe {
				texture.paint_region(
					&self.d3d11_ctx,
					std::mem::transmute::<_, &[u32]>(&pixels[..]),
					sx,
					sy,
					w,
					h,
