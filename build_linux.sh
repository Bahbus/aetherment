if [ "$1" = "client" ] && [ "$2" = "release" ]; then
	...
	DOTNET_CLI_TELEMETRY_OPTOUT="1" dotnet build ./plugin/plugin/Aetherment.csproj -c Release
elif [ "$1" = "plugin" ]; then
	echo "plugin dev"
	...
	DOTNET_CLI_TELEMETRY_OPTOUT="1" dotnet build ./plugin/plugin/Aetherment.csproj -c Debug
else
	echo "invalid target"
fi