...
impl D3d11Renderer {
	...
	pub fn new(device_ptr: usize, texture_register: Box<dyn Fn(&super::Texture) -> u64>) -> Self {
		...
		let compat_disable_depth = Self::env_bool("AETHERMENT_D3D11_COMPAT_DISABLE_DEPTH");
		let compat_disable_cull = Self::env_bool("AETHERMENT_D3D11_COMPAT_DISABLE_CULL");
		log::info!(
			target: "renderer",
			"D3D11 compat settings: disable_depth={}, disable_cull={}",
			compat_disable_depth,
			compat_disable_cull
		);
			...
			device
				.CreateDepthStencilState(
					&D3D11_DEPTH_STENCIL_DESC {
						DepthEnable: windows::core::BOOL((!compat_disable_depth) as i32),
						DepthWriteMask: if compat_disable_depth {
			...
			device
				.CreateRasterizerState(
					&D3D11_RASTERIZER_DESC {
						...
						CullMode: if compat_disable_cull {
							D3D11_CULL_NONE
						} else {
							D3D11_CULL_FRONT
						},
...
impl super::RendererInner for D3d11Renderer {
	fn create_material(&self, shader: &str, binds: &[super::MaterialBind]) -> super::Material {
		...
		let hlsl = {
			...
			let info = naga::valid::Validator::new(
				Default::default(),
				naga::valid::Capabilities::CLIP_DISTANCE | naga::valid::Capabilities::CULL_DISTANCE,
			)
		...
		unsafe {
			device.CreatePixelShader(
				&compile_shader(&hlsl, "fs_main", "ps_5_0").unwrap(),
				None,
				Some(&mut ps),
			)
		}
			...
			device
				.CreateTexture2D(
					&D3D11_TEXTURE2D_DESC {
						...
						Usage: if usage.contains(super::TextureUsage::RENDER_TARGET)
						...
						BindFlags: if usage.contains(super::TextureUsage::TEXTURE_BINDING) {
						...
						CPUAccessFlags: if usage.contains(super::TextureUsage::COPY_DST) {
							D3D11_CPU_ACCESS_WRITE.0 as u32
						} else {
							0
						} | if usage.contains(super::TextureUsage::COPY_SRC) {
			...
			device
				.CreateBuffer(
					&D3D11_BUFFER_DESC {
						...
						BindFlags: if usage.contains(super::BufferUsage::UNIFORM) {
						...
						CPUAccessFlags: if usage.contains(super::BufferUsage::COPY_DST) {
							D3D11_CPU_ACCESS_WRITE.0 as u32
						} else {
							0
						} | if usage.contains(super::BufferUsage::COPY_SRC) {
							D3D11_CPU_ACCESS_READ.0 as u32
						} else {
							0
						},
	...
	fn create_sampler(
		&self,
		address_u: super::SamplerAddress,
		address_v: super::SamplerAddress,
		min: super::SamplerFilter,
		mag: super::SamplerFilter,
	) -> super::Sampler {
		let device = &(self.device_ptr as _);
		let device = unsafe { ID3D11Device::from_raw_borrowed(device).unwrap() };
				...
				.CreateSamplerState(
					&D3D11_SAMPLER_DESC {
						Filter: match (min, mag) {
							(super::SamplerFilter::Nearest, super::SamplerFilter::Nearest) => {
								D3D11_FILTER_MIN_MAG_MIP_POINT
							}
							(super::SamplerFilter::Nearest, super::SamplerFilter::Linear) => {
								D3D11_FILTER_MIN_POINT_MAG_LINEAR_MIP_POINT
							}
							(super::SamplerFilter::Linear, super::SamplerFilter::Nearest) => {
								D3D11_FILTER_MIN_LINEAR_MAG_MIP_POINT
							}
							(super::SamplerFilter::Linear, super::SamplerFilter::Linear) => {
	...
	fn render(
		...
		unsafe {
			if let Some(clear_color) = clear_color {
				self.context
					.ClearRenderTargetView(render_target.view_rt.as_ref().unwrap(), clear_color);
				if !self.compat_disable_depth {
					self.context.ClearDepthStencilView(
						depth_buffer.view_depth.as_ref().unwrap(),
						D3D11_CLEAR_DEPTH.0,
						1.0,
						0,
					);
			...
			self.context.OMSetRenderTargets(
				Some(&[Some(render_target.view_rt.as_ref().unwrap().clone())]),
				depth_buffer.view_depth.as_ref().unwrap(),
			);
			self.context
			...
			for obj in objects {
				...
				let material = materials
					.get(obj.get_material_id())
					.unwrap()
					.as_any()
					.downcast_ref::<D3d11Material>()
					.unwrap();
				...
				device
					.CreateBuffer(
						...
						None,
						Some(&mut uniform_buffer),
					)
					.unwrap();
				let uniform_buffer = uniform_buffer.unwrap();

				let proj =
					camera.proj_matrix(render_target.width as f32 / render_target.height as f32);
				...
				self.context
					.VSSetConstantBuffers(0, Some(&[Some(uniform_buffer.clone())]));
				self.context
					.PSSetConstantBuffers(0, Some(&[Some(uniform_buffer)]));
				self.context.IASetInputLayout(&material.layout);
				for (i, (resource, bind)) in obj
					...
					.enumerate()
				{
					...
					match resource {
						...
						super::ShaderResource::Texture(texture) => {
							...
							if bind.stage.contains(super::MaterialBindStage::VERTEX) {
								self.context
									.VSSetShaderResources(index, Some(&[texture.view.clone()]));
							}

							if bind.stage.contains(super::MaterialBindStage::FRAGMENT) {
								self.context
									.PSSetShaderResources(index, Some(&[texture.view.clone()]));
							}
						}
						...
						super::ShaderResource::Sampler(sampler) => {
							...
							if bind.stage.contains(super::MaterialBindStage::VERTEX) {
								self.context
									.PSSetSamplers(index, Some(&[Some(sampler.sampler.clone())]));
							}
				...
				self.context.IASetVertexBuffers(
					...
					Some(&Some(
						obj.get_vertex_buffer()
							.as_any()
							.downcast_ref::<D3d11Buffer>()
							.unwrap()
							.buffer
							.clone(),
					)),
					Some(&(size_of::<crate::Vertex>() as u32)),
					Some(&0),
				);
			...
			self.context
				.FinishCommandList(true, Some(&mut cmdlist))
				.unwrap();
			let cmdlist = cmdlist.unwrap();
			device
				.GetImmediateContext()
				.unwrap()
				.ExecuteCommandList(Some(&cmdlist), true);
